...
description: 解析用户提供的抖音分享口令或官方链接。用户只发送链接或口令时，下载平台播放源中的无抖音水印原视频；用户要求提取逐字稿、文案或转文字时，在本地识别并只交付校对后的 Markdown 逐字稿。仅用于用户自有或已获授权的内容。
---
...
脚本把下载视频和机器识别稿放在受控临时目录，只返回草稿路径与建议的最终 Markdown 路径。随后必须：

1. 完整读取机器识别稿。
2. 结合标题和上下文修正同音词、专有名词、断句、标点及明显口误，不新增原视频没有的观点。
3. 将正文写入脚本返回的 `final_path`。不要输出 SRT、JSON、TXT 或单独的视频文件。
4. 抽查开头、中段、结尾，确认不是截断稿或空稿。
5. 用脚本清理其返回的 `work_dir`：

```bash
python3 "$SKILL_DIR/scripts/transcribe_douyin.py" --cleanup-work-dir "<work_dir>"
```

最终交付目录只允许保留一个校对后的逐字稿 `.md` 文件。清理失败时必须报告残留路径，不能声称已经只保留成品。
